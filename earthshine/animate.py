...
def RAxisAngle(axis=[0, 1, 0], theta=0):
    ...
    cost = np.cos(theta * np.pi / 180.0)
    sint = np.sin(theta * np.pi / 180.0)
    return np.reshape(
        [
...
def render(map, time, phase0=-56.5, time0=1325.5, res=75, period=0.9972696):
    """
    ...
    map.axis = [0, 1, 0]
    phase = (360.0 * (time - time0) / period) % 360.0 + phase0
    R = RAxisAngle([0, 1, 0], phase)
    map.rotate(phase)
    ...
    axis /= sintheta
    theta = 180.0 / np.pi * np.arctan2(sintheta, costheta)
    R = RAxisAngle(axis, theta)
    ...
    map.axis = [0, 0, 1]
    theta = 180.0 / np.pi * np.arctan2(north_pole[0], north_pole[1])
    map.rotate(theta)
    ...
    # Render the image
    Z = map.render(source=source, res=res).reshape(res, res)
    map.reset()
    map[1:, :] = y
    return Z
...
# Visualize the orbit
def animate(
    map, time, phase0=-56.5, time0=1325.5, res=75, interval=75, period=0.9972696
):
    """
    ...
    norm = 1.0 / np.sqrt(sun[0, i] ** 2 + sun[2, i] ** 2)
    x = sun[0, i] * norm
    y = sun[2, i] * norm
    theta = 180.0 / np.pi * np.arctan2(y, x)
    ...
    norm = 1.0 / np.sqrt(sun[0, i] ** 2 + sun[1, i] ** 2)
    x = sun[0, i] * norm
    y = sun[1, i] * norm
    theta = 180.0 / np.pi * np.arctan2(y, x)
    ...
    norm = 1.0 / np.sqrt(sun[2, i] ** 2 + sun[1, i] ** 2)
    x = sun[2, i] * norm
    y = sun[1, i] * norm
    theta = 180.0 / np.pi * np.arctan2(y, x)
    ...
    for i in tqdm(range(len(time))):
        ...
        map[:, :, :] = y
        map.axis = [0, 1, 0]
        phase = (360.0 * (time[i] - time0) / period) % 360.0 + phase0
        R = RAxisAngle([0, 1, 0], phase)
        map.rotate(phase)
        ...
        # This doesn't actually change the integrated flux!
        map.axis = [0, 0, 1]
        theta = 180.0 / np.pi * np.arctan2(north_pole[i, 0], north_pole[i, 1])
        map.rotate(theta)
        R = RAxisAngle([0, 0, 1], theta)
    ...
    image = ax[0, 1].imshow(
        Z[0], extent=(-1, 1, -1, 1), origin="lower", cmap=cmap, vmin=vmin, vmax=vmax
    )
...
if __name__ == "__main__":
    map = starry.Map(ydeg=15, reflected=True, lazy=False)
    map.load("earth")
