...
priors used. 
We present results in \S\ref{sec:results} and make comparison of these 
findings to state-of-the-art terrestrial planet maps from the literature in 
\S\ref{sec:discussion}. 
Finally, we conclude in \S\ref{sec:conclusions} with a look at future 
...
As with all \TESS time series data products, the time is given in \TESS Julian 
Days (TJD), defined as $\mathrm{BJD} - 2457000$, and the data are delivered in 27-day 
...
Sol d is inhomogenously spread across the \TESS detectors and manifests 
...
To accomplish this, we... \todo{(decide whether it's outlier rejection only or 
full-on categorization)}

\todo{(masking out epochs without Sol d shine, subtraction of baseline 
background flux)}

The final data set to be used consists of 75 Sector 1 background lightcurves 
and 20 Sector 2 background lightcurves corresponding to the epochs at which 
Sol d signal was clearly registered by \TESS. 
...
in the \TESS detector. We detect three large scale bright regions near
...
significant latitudinal extent, likely tracking oceans. Because of the
