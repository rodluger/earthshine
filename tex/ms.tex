...
and $\mathbf{y}$ is the vector of spherical harmonic coefficients. 
...
As we will see below, the linearity of the problem makes it easily invertible.
...
systematics model $\mathbf{b}_n$ (different for each target) and
the \starry model $\mathbf{a}$ (shared by all targets). Since we happen
...
underfits the data. In the next step, we remove our L2 prior on the spherical
...
side. This constraint breaks the linearity of the problem, so we must
now use a non-linear optimizer.
