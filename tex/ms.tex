...
Of the many challenges and opportunities facing the field of exoplanets in the future, the prospect of mapping the surface of an Earth-like planet is one of the most exciting -- and one of the most difficult. 
...
Once these targets are found, efforts to characterize them in detail and determine their habitability will begin in earnest. 
The discovery of surface features including continents and oceans will be a critical hint for the habitability of an alien world.

Such mapping can be done by constructing a phase map of the planet using precise photometry obtained over long periods of time. 
\todo{(literature background \& citations)}

\todo{We need to cite} \citep{Berdyugina2017,Cowan2009,Cowan2018,Haggard2018,Kawahara2010,Fujii2012,Lustig-Yaeger2018} and others.

\todo{We need to cite}\citep{Knutson2007,Majeau2012,deWit2012} on the first maps of an exoplanet.
...
the northern Atlantic Ocean, which appears to extends into the east coast of
North America. We identify three broad reasons for this.
...
of the complex optics that give rise to the signal in the first place, and our
attempt to na{\"i}vely model it with pixel-level decorrelation, likely
introduces significant correlated noise into the problem. We discuss this
...
Third, and most importantly, we have not performed actual inference: that is, 
...
the project we used as regressors actual \tess housekeeping variables---such as the angle 
...
in the background flux of \tess pixels. Because of its small size and
...
where \tess is closer to Sol d I than to Sol d. While this will
...
signal of Sol d: additionally, there is likely temporal variation in the 
...
Observed in the \tess band, Sol d is a planet dominated by
clouds. \todo{Discuss how this problem is *hard* and how we need
large time baseline to disentangle clouds from continents, and even
then there are cloud features that are stationary, so hard to
tell apart. What we really need is multi-band photometry,
as suggested in \cite{Cowan2009}.}
